...
sha256_validator = RegexValidator(
    r"\A[0-9a-f]{64}\Z", "Enter a lowercase SHA-256 digest."
)
...
class Project(models.Model):
    ...
    visibility = models.CharField(
        max_length=10, choices=Visibility, default=Visibility.MEMBERS
    )
    is_archived = models.BooleanField(default=False)
    require_download_verification = models.BooleanField(default=False)
    current_upload = models.OneToOneField(
        "ProjectUpload",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
        related_name="current_for_project",
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                Lower("slug"), F("owner"), name="hosting_owner_slug_unique"
            )
        ]
    ...
    def clean(self):
        ...
        if self.current_upload_id:
            ...
            if (
                upload.project_id != self.pk
                or upload.status != ProjectUpload.Status.READY
            ):
                raise ValidationError(
                    {
                        "current_upload": "The current upload must be ready and belong to this project."
                    }
                )
...
class ProjectMembership(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="hosting_memberships",
    )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["project", "user"], name="hosting_member_unique"
            )
        ]
...
class ProjectUpload(models.Model):
    ...
    class Meta:
        indexes = [
            models.Index(fields=["project", "status"], name="hosting_upload_status_idx")
        ]
        constraints = [
            models.CheckConstraint(
                condition=~Q(status="ready")
                | (Q(ready_at__isnull=False) & ~Q(sha256="")),
                name="hosting_ready_metadata",
            )
        ]
...
class ProjectFile(models.Model):
    upload = models.ForeignKey(
        ProjectUpload, on_delete=models.CASCADE, related_name="files"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["upload", "path"], name="hosting_file_path_unique"
            )
        ]
...
class AccessToken(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="hosting_tokens",
    )
    name = models.CharField(max_length=100)
    token_hash = models.CharField(
        max_length=64, unique=True, validators=[sha256_validator], editable=False
    )
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                condition=Q(expires_at__gt=F("created_at")), name="hosting_token_expiry"
            )
        ]
...
class VerificationRequest(models.Model):
    ...
    token = models.ForeignKey(
        AccessToken, on_delete=models.CASCADE, related_name="verifications"
    )
    upload = models.ForeignKey(
        ProjectUpload, on_delete=models.CASCADE, related_name="verifications"
    )
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                ...
                name="hosting_verify_expiry",
            ),
            models.CheckConstraint(
                condition=~Q(status="approved") | Q(verified_at__isnull=False),
                name="hosting_verify_approved",
            ),
        ]
...
class DownloadGrant(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    token = models.ForeignKey(
        AccessToken, on_delete=models.CASCADE, related_name="download_grants"
    )
    upload = models.ForeignKey(
        ProjectUpload, on_delete=models.CASCADE, related_name="download_grants"
    )
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                condition=Q(expires_at__gt=F("created_at")), name="hosting_grant_expiry"
            )
        ]
    ...
    def clean(self):
        ...
        if self.verification_id:
            ...
            if (
                verification.token_id != self.token_id
                or verification.upload_id != self.upload_id
            ):
                raise ValidationError(
                    {"verification": "Verification must match the token and upload."}
                )
...
class TransferEvent(models.Model):
    ...
    project = models.ForeignKey(
        Project, null=True, on_delete=models.SET_NULL, related_name="transfer_events"
    )
    ...
    class Meta:
        indexes = [
            models.Index(
                fields=["user", "action", "created_at"], name="hosting_event_usage_idx"
            )
        ]
