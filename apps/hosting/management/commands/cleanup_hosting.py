...
from apps.hosting.models import ProjectUpload
    ...
    def handle(self, *args, **options):
        ...
        for path in root.glob("*"):
            if (
                path.is_symlink()
                or not path.is_file()
                or path.stat().st_mtime >= cutoff.timestamp()
            ):
                continue
            if path.name.startswith("incoming-"):
                ...
                removable = upload is None or (
                    upload.status
                    in (ProjectUpload.Status.FAILED, ProjectUpload.Status.SUPERSEDED)
            ...
            if removable:
                ...
                count += 1
        self.stdout.write(
            f"{'Would remove' if options['dry_run'] else 'Removed'} {count} obsolete archives/staging files."
        )
