from home.models import Comment, CommentLike, Thread, ThreadLike, ThreadView
...
def collect_user_events(user):
    ...
    for comment_like in CommentLike.objects.select_related("comment__thread").filter(user=user):
        events.append({
    ...
    return sorted(events, key=lambda event: event["created_at"], reverse=True)[:MAX_EVENTS]
