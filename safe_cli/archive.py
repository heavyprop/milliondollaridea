...
def inspect_archive(
    path, *, max_archive=MAX_ARCHIVE, max_unpacked=MAX_UNPACKED, max_files=MAX_FILES
):
            ...
            for entry in entries:
                ...
                for length in range(1, len(parts) + 1):
                    ...
                    if previous is not None and previous != (prefix, expected):
                        raise ArchiveError(
                            "Archive contains conflicting file/directory paths."
                        )
                    nodes[prefix.casefold()] = (prefix, expected)
                mode = stat.S_IFMT(entry.external_attr >> 16)
                allowed_modes = (
                    (0, stat.S_IFDIR) if entry.is_dir() else (0, stat.S_IFREG)
                )
                ...
                if actual != entry.file_size:
                    raise ArchiveError(
                        "Archive entry size does not match its contents."
                    )
                try:
                    sample.decode("utf-8")
